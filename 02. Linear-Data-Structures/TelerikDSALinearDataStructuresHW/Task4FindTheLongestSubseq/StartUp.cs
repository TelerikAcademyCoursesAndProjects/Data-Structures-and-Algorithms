...
namespace Task4FindTheLongestSubseq
{
    ...
    {
        internal static List<int> FindBestSubsequence(List<int> sequence)
        {
        ...
        {
            Console.WriteLine("Enter integer numbers on a single line separated by ,");
            var sequenceOfNumbers = Console.ReadLine().Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToList();
            var longestSubsequence = FindBestSubsequence(sequenceOfNumbers);
